...
export default function AdminLocations() {
  ...
  const handleSeedSample = async () => {
    try {
      setSeeding(true);
      const res = await fetch('/api/admin/locations/seed-sample', { method: 'POST' });
      if (!res.ok) return;
      await fetchLocations();
    } catch (e) {
      console.error('Failed to seed sample location:', e);
    } finally {
      setSeeding(false);
    }
  };
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={seeding}
                className="flex items-center gap-2 px-4 py-3 bg-white text-blue-700 border border-blue-200 hover:bg-blue-50 rounded-xl font-medium transition-colors duration-200 text-sm md:text-base disabled:opacity-60"
                aria-label="Seed Sample Location"
                title="Create a sample location with slots"
              >
                {seeding ? (
                  <span className="inline-block w-4 h-4 border-2 border-blue-600 border-t-transparent rounded-full animate-spin" />
                ) : (
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v1m0 14v1m7-7h1M3 12H2m14.95 4.95l.707.707M6.343 6.343l-.707-.707m0 12.728l.707-.707M17.657 6.343l-.707.707" />
                  </svg>
                )}
                Seed Sample
              </button>
              <button
                ...
                className="flex items-center gap-2 px-5 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-xl font-medium transition-all duration-300 text-sm md:text-base"
              ...
              <div className="mb-6">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-gray-900">
                    ${location.pricing.min.toFixed(2)} - ${location.pricing.max.toFixed(2)}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">Average:</span>
                  <span className="font-medium text-blue-600">${location.pricing.average.toFixed(2)}</span>
                </div>
              </div>
...
function LocationModal({ 
  ...
  const [formData, setFormData] = useState({
    ...
    initialSlotsCount: location ? location.availability?.total?.toString() || '' : '',
    defaultBasePrice: location ? location.pricing?.min?.toString() || '' : '',
    slotType: 'STANDARD',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await fetch(url, {
      ...
      // include optional initial slot setup
      initialSlotsCount: formData.initialSlotsCount ? Number(formData.initialSlotsCount) : 0,
      defaultBasePrice: formData.defaultBasePrice ? Number(formData.defaultBasePrice) : null,
      slotType: formData.slotType || 'STANDARD',
        }),
      });
  ...
  <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {!location && (
            <div className="border-t pt-4">
              <h4 className="text-sm font-semibold text-gray-900 mb-3">Initial Slots (optional)</h4>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <input
                    ...
                    min={0}
                    placeholder="e.g. 20"
                    value={formData.initialSlotsCount}
                    onChange={(e) => setFormData(prev => ({ ...prev, initialSlotsCount: e.target.value }))}
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-300"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData(prev => ({ ...prev, defaultBasePrice: e.target.value }))}
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-300"
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setFormData(prev => ({ ...prev, slotType: e.target.value }))}
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-300"
                  >
              ...
              <p className="text-xs text-gray-500 mt-2">We’ll auto-create numbered slots (S-001, S-002, …) with the selected type and price.</p>
            </div>
          )}
