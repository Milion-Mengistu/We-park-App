...
export default function AdminReports() {
    ...
    try {
      ...
      setReportData({
        ...
        topLocations: [
          ...
          { name: 'City Center Garage', revenue: 4680.5, bookings: 87 },
          { name: 'Business District', revenue: 3420.0, bookings: 65 },
          { name: 'Mall Parking', revenue: 2399.0, bookings: 92 },
        ],
        dailyStats: [
          {
            date: '2024-01-15',
        ...
        paymentMethods: [
          { method: 'TELEBIRR', count: 145, amount: 6780.5 },
          { method: 'CBE_BIRR', count: 98, amount: 4520.75 },
          { method: 'CHAPA', count: 67, amount: 3210.0 },
          { method: 'CASH', count: 32, amount: 1239.0 },
        ],
        monthlyTrends: [
          { month: 'Nov 2024', revenue: 14250.0, bookings: 298 },
  ...
  const handleExportReport = async (format: 'csv' | 'pdf') => {
    try {
      ...
      const response = await fetch(
        `/api/admin/reports/export?format=${format}&period=${selectedPeriod}`,
        {
          method: 'POST',
        }
      );
      ...
      const mockData = `Date,Revenue,Bookings,Occupancy\n${reportData?.dailyStats
        .map((d) => `${d.date},$${d.revenue},${d.bookings},${d.occupancy}%`)
        .join('\n')}`;
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-white/50 shadow-lg">
            ...
            <div className="space-y-4">
              {reportData.paymentMethods.map((method) => {
                const percentage =
                  (method.count / reportData.totalBookings) * 100;
                return (
                  <div
                    key={method.method}
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-white/50 shadow-lg mb-8">
          ...
          <div className="overflow-x-auto">
            <div
              className="flex items-end gap-4 h-64 p-4"
              style={{ minWidth: '600px' }}
            >
              {reportData.dailyStats.map((day) => {
                const maxRevenue = Math.max(
                  ...reportData.dailyStats.map((d) => d.revenue)
                );
