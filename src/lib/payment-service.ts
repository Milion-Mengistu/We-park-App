...
export class PaymentService {
  ...
  private static async callTelebirrAPI(params: any): Promise<any> {
    ...
    // Dummy mode: always succeed and include masked metadata
    const dummyMode = process.env.TELEBIRR_DUMMY_MODE === 'true' 
      || !process.env.TELEBIRR_APP_KEY 
      || !process.env.TELEBIRR_MERCHANT_ID;

    const mask = (v?: string) => (v ? `${v.slice(0, 3)}***${v.slice(-3)}` : 'not-set');

    if (dummyMode) {
      return {
        success: true,
        transactionId: `TB-DUMMY-${Date.now()}-${Math.random().toString(36).substr(2, 6)}`,
        status: 'COMPLETED',
        amount: params.amount,
        gateway: 'TELEBIRR_DUMMY',
        meta: {
          merchantId: mask(process.env.TELEBIRR_MERCHANT_ID),
          appId: mask(process.env.TELEBIRR_APP_ID),
        },
      };
    }

    // Non-dummy simulation: retain mild randomness but mostly succeed
    if (Math.random() > 0.05) {
      return {
        success: true,
